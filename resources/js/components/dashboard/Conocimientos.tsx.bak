...
export default function Conocimientos({
  ...
  const uploadDocument = async (file: File, category: string) => {
    ...
    try {
      ...
      const companyId = company?.id || user?.company_id;
      const response = await fetch(
        "http://n8n-admin.withmia.com/webhook/upload-document",
        {
          method: "POST",
  ...
  const deleteDocument = async (docId: string) => {
    if (!confirm("┬┐Est├ís seguro de eliminar este documento?")) return;
      ...
      <div className="flex items-center space-x-4 mb-8">
        ...
        <div>
          ...
          <p className="text-lg text-neutral-500">
            Base de conocimiento y documentaci├│n de tu empresa
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-lg">
        ...
        >
          ...
          <p className="text-sm text-gray-500 mb-4">
            Tipos soportados: PDF, TXT, DOCX, MD (m├íximo 10MB)
          </p>
