...
// Ô£¿ OPTIMIZACIONES: Utilidades extraídas para mejor performance
...
const ConversationsInterface: React.FC = () => {
  ...
  // 7. Contact Info Panel (Panel de información)
  ...
  const debouncedFetchConversations = useCallback(() => {
    ...
    if (isPendingReloadRef.current) {
      console.log('⏸️ Reload ya en progreso, agendando siguiente...');
      return;
    }
    ...
    if (timeSinceLastReload < 2000) {
      ...
      console.log(`⏱️ Agendando reload en ${delay}ms...`);
  ...
  // ­ƒÄ» Auto-focus del input cuando se abre una conversación (como WhatsApp)
  useEffect(() => {
  ...
  const {
    ...
    onNewMessage: async (event) => {
      ...
      console.log('🔍 Conversación activa ID:', activeConversationRef.current?.id);
      ...
      if (processedEventsRef.current.has(eventId)) {
        console.log('⏭️ Evento duplicado, omitiendo');
        return;
      }
      ...
      setConversations((prevConversations: any[]) => {
        ...
        const updated = prevConversations.map((conv: any) => {
        ...
        const sorted = updated.sort((a: any, b: any) => {
          const timeA = a.timestamp || new Date(a.updated_at || 0).getTime() / 1000;
          const timeB = b.timestamp || new Date(b.updated_at || 0).getTime() / 1000;
          return timeB - timeA;
        });
      ...
      if (event?.message && event?.conversation) {
        ...
        console.log('✅ Notificación enviada para nuevo mensaje');
      }
  ...
  // Ô£à NUEVO: Referencias para virtualización
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const mappedResults = response.data.data.map((conv: any) => {
            // Buscar avatarUrl en múltiples ubicaciones posibles
          ...
          setSearchResults([]);
          console.warn('⚠️ Búsqueda sin resultados');
        }
      } catch (error: any) {
  ...
  const filteredConversations = React.useMemo(() => {
    ...
    return result;
  }, [conversations, searchTerm, selectedFilter, searchResults?.length ?? 0]);
  ...
  const handleSelectConversation = async (conversation: Conversation) => {
    ...
    // ÔÅ▒´©Å Esperar 2 segundos antes de marcar como leída (para que se vea la notificación)
    setTimeout(() => {
      if (activeConversationRef.current?.id === conversation.id) {
        // Todavía est├í en esta conversación, marcar como leída
        const updatedConversations = conversations.map(conv =>
          conv.id === conversation.id ? { ...conv, unread_count: 0 } : conv
        );
        // No actualizamos el estado aquí porque ya se marca en useChatwoot
      }
    }, 2000);
  };
  ...
  // 8´©ÅÔâú MUTE - Silenciar conversación
  ...
  const handleStartRecording = async () => {
    try {
      ...
      console.error('ÔØî Error al iniciar grabación:', error);
  ...
  // 3´©ÅÔâú MULTIMEDIA GALLERY - Extraer multimedia de la conversación
  ...
  // Ô£à NUEVO: Función para hacer scroll al final de los mensajes
  const scrollToBottom = useCallback((behavior: 'smooth' | 'auto' = 'auto') => {
    const container = messagesContainerRef.current;
    debugLog.log('⬇️ scrollToBottom llamado - behavior:', behavior, 'container existe:', !!container);
    if (container) {
      ...
      if (behavior === 'smooth') {
        ...
        // Scroll instantáneo más confiable
  ...
  // Ô£à NUEVO: Auto-scroll cuando se carga una conversación o llegan mensajes nuevos
  useEffect(() => {
    if (activeConversation?.messages && (activeConversation?.messages || []).length > 0) {
      ...
      if (previousMessageCount > 0 && currentMessageCount > previousMessageCount) {
        ...
        // Hay mensajes nuevos - mostrar separador en el índice del ├║ltimo mensaje antiguo
  ...
  useEffect(() => {
    ...
    debugLog.log('👁️ Iniciando observer para mantener scroll al fondo');

    // Función para forzar scroll al fondo
    const scrollToBottomImmediate = () => {
      // ­ƒÜ½ NO hacer scroll autom├ítico si hay b├║squeda activa
      if ((searchResults || []).length > 0) {
        debugLog.log('­ƒöì B├║squeda activa - cancelando auto-scroll');
        return;
      }
    ...
    const observer = new MutationObserver(() => {
      debugLog.log('­ƒöä DOM mutado - haciendo scroll al fondo');
    ...
    // Cleanup
    return () => {
      debugLog.log('­ƒøæ Deteniendo observer de scroll');
      observer.disconnect();
    };
  ...
  useEffect(() => {
    if (searchTerm && searchResults && activeConversation?.messages) {
      ...
      setTimeout(() => {
        // Buscar el primer mensaje que contiene el término
  ...
  // Ô£à Resetear contador cuando cambia de conversación
  useEffect(() => {
    setPreviousMessageCount(0);
    setNewMessageSeparatorIndex(null);
    // ­ƒåò Limpiar b├║squeda al cambiar de conversación
    setSearchQuery('');
    setMessageSearchResults([]);
    setCurrentSearchIndex(0);
  ...
  const handleOpenEditModal = () => {
    if (activeConversation) {
      ...
      console.log('­ƒöì DEBUG - Abriendo modal de edición:', {
      ...
      // Si name es solo el n├║mero, dejamos el campo nombre vacío para que el usuario lo complete
      ...
      console.error('ÔØî No hay conversación activa');
    }
  };
  ...
  const handleSaveContact = async () => {
    ...
    try {
      ...
      const updateResponse = await fetch(`/api/chatwoot-proxy/contacts/${contactId}`, {
        ...
        body: JSON.stringify({
          name: editName || editPhone, // Si no hay nombre, usar el teléfono
          email: editEmail,
      ...
      // Ocultar toast de error después de 3 segundos
      setTimeout(() => {
        setShowToast(false);
      }, 3000);
    }
  ...
  const handleDownloadChat = () => {
    ...
    try {
      console.log('­ƒôÑ Iniciando descarga de conversación:', activeConversation.id);
      ...
      const infoData = [
        ...
        [''],
        ['ID Conversación:', activeConversation.id],
        ...
        ['Mensajes no leídos:', activeConversation.unread_count || 0],
  ...
  // Ô£à Función mejorada con modal de progreso
  const handleDownloadAllConversations = async () => {
    try {
      ...
      console.log('­ƒôÑ Solicitando exportación de conversaciones con mensajes...');
      ...
      setDownloadStatus('Organizando información de contactos...');
  ...
  // Ô£¿ OPTIMIZACI├ôN: Funciones extraídas a ../utils/ para mejor performance y reutilización
        ...
        <div className="p-4 border-b border-gray-200/40 bg-white/60 backdrop-blur-xl shadow-sm">
          ...
          <div className="flex space-x-2">
            {[
              ...
              { id: 'mine', label: 'Mío', count: (conversations || []).filter(c => c.assignee_id === 1).length },
        ...
        {activeConversation ? (
          <>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2" onClick={(e) => e.stopPropagation()}>
                  ...
                  {/* 🎯 Controles de navegación entre coincidencias */}
            ...
            >
              ...
              {isDragging && (
                <div className="absolute inset-0 z-50 bg-blue-500/20 backdrop-blur-sm border-4 border-dashed border-blue-500 rounded-lg flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-xl font-semibold text-blue-800">Suelta los archivos aquí</p>
              ...
              {/* Ô£à Renderizar mensajes (sin duplicados) */}
                ...
                <React.Fragment key={message.id}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 relative">
                      ...
                      {/* Botón DESPU├ëS de la burbuja (para mensajes propios) */}
                      {message.sender === 'agent' && (
                        <button
                          onClick={() => setMessageMenuOpen(messageMenuOpen === message.id ? null : message.id)}
                          className="p-1.5 rounded-full bg-white/80 hover:bg-white shadow-sm opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            <>
              <div className="text-center">
                ...
                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium mt-2 ${
                  ...
                  {activeConversation.contact.status === 'online' ? 'En línea' : 'Desconectado'}
                </span>
              </div>
